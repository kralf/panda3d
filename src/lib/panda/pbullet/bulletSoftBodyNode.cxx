...
sync_b2p() {
  ...
  if (_geom) {
    ...
    while (!vertices.is_at_end()) {
      ...
      vertices.set_data3f((PN_stdfloat)v.getX(), (PN_stdfloat)v.getY(), (PN_stdfloat)v.getZ());
      normals.set_data3f((PN_stdfloat)n.getX(), (PN_stdfloat)n.getY(), (PN_stdfloat)n.getZ());
    }
  }
...
make_tet_mesh(BulletSoftBodyWorldInfo &info, const char *ele, const char *face, const char *node) {
  ...
  for (int i=0; i<pos.size(); ++i) {
    ...
    sscanf(node, "%d %lf %lf %lf", &index, &x, &y, &z);
    node += next_line(node);
